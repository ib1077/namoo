...
async function loadBook() {
  try {
    ...
    try {
      ...
      throw new Error("book.json または book.txt を読み込めませんでした");
    }
  }

  pages = pages
    .map(page => ({ image: String(page.image || "").trim(), text: String(page.text || "").trim() }))
    .filter(page => page.image && page.text);

  if (!pages.length) throw new Error("絵本に有効なページがありません");
}
...
function waitForImage(src) {
  return new Promise(resolve => {
    ...
    ui.image.onload = () => { clearTimeout(timeout); done(); };
    ui.image.onerror = () => { clearTimeout(timeout); done(); };
    ui.image.src = src;
    if (ui.image.complete) { clearTimeout(timeout); done(); }
  });
}
...
function speak(text, token) {
  return new Promise(resolve => {
    if (!playing || token !== runId) return resolve();
...
async function playPage(token) {
  ...
  ui.number.textContent = `${currentPage + 1} / ${pages.length}`;
  ui.image.alt = `${currentPage + 1}ページ目の絵`;
  ...
  const continued = await delay(isLast ? SETTINGS.loopPauseMs : SETTINGS.pauseBetweenPagesMs, token);
...
function stopPlayback(showResume = true) {
  ...
  if (showResume) ui.resumeScreen.hidden = false;
}
...
ui.startButton.addEventListener("click", startPlayback);
ui.pauseButton.addEventListener("click", () => stopPlayback(true));
ui.resumeButton.addEventListener("click", startPlayback);

document.addEventListener("visibilitychange", async () => {
  if (document.visibilityState === "visible" && playing) {
    await requestWakeLock();
    // iOSはバックグラウンド移行後に読み上げが止まるため、現在ページから安全に再開する。
    speechSynthesis.cancel();
    runId += 1;
    playPage(runId);
  }
...
loadBook().then(() => {
  ui.status.textContent = `${pages.length}ページの えほんです`;
